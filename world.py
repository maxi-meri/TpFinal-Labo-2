...
class World():
        ...
        for y, row in enumerate(data):
            for x, tile in enumerate(row):
                ...
                #Hongo
                    ...
                    hongo = EnemigoDisparo(image_x, image_y, animations_enemies[1], consts.ENERGIA_HONGOS, 3)
    ...
    def abrir_puerta(self, player, tile_list, puede_abrir=True):
        # Las puertas solo se abren cuando puede_abrir es True.
        # En main se manda True cuando ya no quedan enemigos en la ronda/sala.
        if not puede_abrir:
            return False

        buffer = 50
        proximidad_rect = pygame.Rect(
            player.shape.x - buffer,
            player.shape.y - buffer,
            player.shape.width + 2 * buffer,
            player.shape.height + 2 * buffer
        )

        for tile_data in self.map_tiles:
            image, rect, x, y, tile_type = tile_data

            if proximidad_rect.colliderect(rect) and tile_type in puerta_cerrada:
                # 163 -> puerta abierta 162 / 178 -> puerta abierta 177
                if tile_type == 163:
                    new_tile_type = 162
                elif tile_type == 178:
                    new_tile_type = 177
                ...
                if tile_data in self.puerta_cerrada_tile:
                    self.puerta_cerrada_tile.remove(tile_data)
