...
#Animacion del boss final
for img in animations_enemies[1]:  # shroom
...
#Agrego boss final a la lista junto a todos los enemigos
if nivel == 2:
    boss = FinalBoss(500, 300, boss_animations)
...
while run:
    ...
    if player.vivo:
        ...
        if (nivel == consts.MAX_LVL and len(lista_enemigos) == 0 and not boss_creado):
            boss = FinalBoss(500, 300, boss_animations)
        ...
        # Balas del Boss
        for bala in grupo_balas_enemigas:

            bala.update()
            window.blit(bala.image, bala.rect)

            if bala.rect.colliderect(player.shape):
                player.energia -= 10
                bala.kill()

            if (bala.rect.right < 0 or bala.rect.left > consts.ANCHO_VENTANA or bala.rect.bottom < 0 or bala.rect.top > consts.ALTO_VENTANA):
                bala.kill()
